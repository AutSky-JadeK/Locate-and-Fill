...
def cona_outputs(outputs):
    res = ''
    ii = 0
    for wds in outputs:
        sent = cona(wds)
        res = res + ('<extra_id_' + str(ii) + '> ')
        res = res + (sent + ' ')
        ii += 1
    return res + ('<extra_id_' + str(ii) + '>')

def getsns_jia(sen, label):
    sns = []
    sn = []
    be_replaced = []
    s_with_mask = []
    ...
    for i in range(len(sen)):
        if label[i] == 'O':
            ...
            if label[i+1] != 'I-REP':
                ...
                '''
                #sns.append(sn)
                sns[-1] = sns[-1] + (['<extra_id_' + str(ii) + '>', '('] + be_replaced + [')'])
                ii += 1
                sn = sn + be_replaced
                s_with_mask.append('[MASK_r]')
                be_replaced = []
    #if sen == ['what', 'else', 'did', 'he', 'do', 'after', 'nba', '[EOS]']:
    #    print(sns)
...
# In[11]:
fo_1 = open(dataset_name + '-becky-prompt-5571-T5-nosplit.json', 'w', encoding='utf-8')
fo_2 = open(dataset_name + '-becky-prompt-5571-T5-ids-nosplit.txt', 'w', encoding='utf-8')
fo_3 = open(dataset_name + '-becky-prompt-5571-T5-inputs-nosplit.txt', 'w', encoding='utf-8')
...
for line in content:
    ...
    sns, s_with_mask = getsns_jia(prta, label)
    #sns_jia, _ = getsns_jia(prta, label)
    ...
    #for i in range(len(sns_jia)):
    #    ts = cona(sns_jia[i])
    #    newtmps.append(ts.replace('<extra_id_0> ( )', cona(tmps[i])))
    
    for i in range(len(sns)):
        if '<extra_id_0>' not in sns[i]:
            continue
        prt={'text': lin + ' [SEP] ' + cona(sns[i]),
        'summary': cona_outputs(tmps)}
        #prt={'text': lin + ' [SEP] ' + cona(sns[i]),
        #'summary': newtmps[i]}
