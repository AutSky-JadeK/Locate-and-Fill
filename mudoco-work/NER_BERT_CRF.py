...
def set_work_dir(local_path="ner_bert_crf", server_path="ner_bert_crf"):
    os.chdir('/home/lizitong/ner_bert_crf')


def get_data_dir(local_path="ner_bert_crf", server_path="ner_bert_crf"):
    return '/home/lizitong/ner_bert_crf'
...
output_dir = './output_mudoco_becky/'
...
class CoNLLDataProcessor(DataProcessor):
    ...
    def __init__(self):
        self._label_types = [ 'X', '[CLS]', '[SEP]', 'O', 'B-REP', 'I-REP', 'B-INS']
    ...
    def get_train_examples(self, data_dir):
        return self._create_examples(
            self._read_data(os.path.join(data_dir, "mudoco_train-becky-bertcrf.txt")))

    def get_dev_examples(self, data_dir):
        return self._create_examples(
            self._read_data(os.path.join(data_dir, "mudoco_dev-becky-bertcrf.txt")))

    def get_test_examples(self, data_dir):
        return self._create_examples(
            self._read_data(os.path.join(data_dir, "mudoco_test-becky-bertcrf.txt")))
...
tokenizer = BertTokenizer.from_pretrained('/home/lizitong/bert-base-cased-original', do_lower_case=do_lower_case)
...
bert_model = BertModel.from_pretrained('/home/lizitong/bert-base-cased-original')
...
fo = open('predicted_ner_labels_mudoco_becky.txt', 'w')
...
with torch.no_grad():
    ...
    for batch in demon_dataloader:
        ...
        valid_predicted = torch.masked_select(predicted_label_seq_ids, predict_mask)
        # valid_label_ids = torch.masked_select(label_ids, predict_mask)
        for i in range(len(predicted_label_seq_ids)):
            ...
            prt = list(map(lambda i: label_list[i], new_ids))
            for j in range(len(prt)):
                if j != 0:
                    print(' ', end='', file=fo)
                print(prt[j], end='', file=fo)
            print('', file=fo)
